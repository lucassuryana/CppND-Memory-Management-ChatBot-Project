...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename) {
                    ...
                    if (type->second == "EDGE") {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end()) {
                            ...
                            auto parentNode = std::find_if(
                                _nodes.begin(), _nodes.end(), [&parentToken](const std::unique_ptr<GraphNode> &node) {
                            ...
                            // create new edge
                            std::unique_ptr<GraphEdge> edge = std::make_unique<GraphEdge>(id); // CHANGES: Use std::make_unique to create GraphEdge
                            ...
                            // transfer ownership of edge to parent and child nodes
                            (*childNode)->AddEdgeToParentNode(edge.get()); // Child node holds non-owning reference
                            (*parentNode)->AddEdgeToChildNode(std::move(edge)); // Parent node takes ownership using std::move
                        }
                    }
                } else {
    ...
    std::cout << "Creating ChatBot on the stack" << std::endl; // CHANGES: Print message indicating stack allocation
    ...
    std::cout << "Moving ChatBot to the root node" << std::endl; // CHANGES: Print message indicating move operation
    bot.SetRootNode(rootNode);
    rootNode->MoveChatbotHere(std::move(bot));

    // Note: As 'bot' goes out of scope here, ChatBot's destructor is called on the stack
